...
const FarmerDashboard = () => {
  ...
  const getAgentStatus = (type) => {
    ...
    return "active"; // Force it to active if bought
  };
  ...
  const aiHubEnabled = activeAgents.length > 0;
        ...
        <Card>
          ...
          <CardContent className="grid gap-4 md:grid-cols-3">
            ...
            <div className="rounded-xl bg-wheat-light p-4">
              ...
              <Link to={aiHubEnabled ? "/consult" : "#"}>
                <Button
                  size="sm"
                  variant="outline"
                  disabled={!aiHubEnabled}
                  onClick={() => aiHubEnabled && handleOpenAIHub()}
                >
                  Open AI Hub
                </Button>
              </Link>
                ...
                <div key={agent._id} className="rounded-lg border p-4 space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 min-w-0">
                      {/* ICON */}
                      <div
                        className={`w-10 h-10 shrink-0 rounded-lg flex items-center justify-center ${colorClass}`}
                      >
                        <Icon className="w-5 h-5" />
                      </div>

                      {/* NAME — kept on same line as icon */}
                    ...
                    {/* STATUS BADGE */}
                    <Badge className={`ml-2 shrink-0 ${statusClass}`}>
                      {status === "active"
                        ? "Active"
                        : "Buy"}
                    </Badge>
